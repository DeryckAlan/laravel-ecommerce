...
class RatingController extends Controller
{
    public function add(Request $request)
    {
		...
		if($product_check)
		{
			$verify_purchace = Order::where('orders.user_id', Auth::id())->join('order_items', 'order_id', 'order_items.order_id')->where('order_items.prod_id', $product_id)->get();
			...
			if($verify_purchace)
			{
				...
				if($existing_rating)
				{
					$existing_rating->status_rated = $stars_rated;
